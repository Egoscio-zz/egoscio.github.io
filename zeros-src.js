...
function zeros(exp, logger) {
	...
	logger(`Factors of P are +-{${pf.reduce((a, b) => `${a}, ${b}`)}}`)
	logger(`Factors of Q are +-{${qf.reduce((a, b) => `${a}, ${b}`)}}`)
	...
	logger(`Possible zeros are +-{${Array.from(pq).reduce((a,b) => `${a}, ${b}`)}}`)
	...
	if (Array.from(zeros).length === 0) {
		...
		logger(`The zeros are: ${Array.from(zeros).reduce((a, b) => `${a}, ${b}`)}`)
	}
}
